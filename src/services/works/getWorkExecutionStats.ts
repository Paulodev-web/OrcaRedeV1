...
export interface WorkExecutionStats {
  /** Metros de rede lancados, somados por categoria. */
  metersByCategory: { BT: number; MT: number; iluminacao: number };
  metersTotal: number;
  /**
...
const VAZIO: WorkExecutionStats = {
  metersByCategory: { BT: 0, MT: 0, iluminacao: 0 },
  metersTotal: 0,
  lastRecordAt: null,
};
 ...
 *
 * Le as tres origens de execucao (poste, equipamento, trecho). Nao usa
...
export async function getWorkExecutionStats(
  ...
  const [spansRes, polesRes, equipRes] = await Promise.all([
    supabase
      .from('work_network_spans')
      .select('work_id, category, meters, installed_at')
      .in('work_id', ids),
    supabase
  ...
  if (spansRes.error || polesRes.error || equipRes.error) {
  ...
  const garante = (workId: string): WorkExecutionStats => {
    if (!out[workId]) {
      out[workId] = {
        metersByCategory: { ...VAZIO.metersByCategory },
        metersTotal: 0,
        lastRecordAt: null,
      };
    }
  ...
  for (const row of (spansRes.data ?? []) as Array<{
    work_id: string;
    category: 'BT' | 'MT' | 'iluminacao';
    meters: number | string | null;
    installed_at: string | null;
  }>) {
    const stats = garante(row.work_id);
    const metros = Number(row.meters ?? 0);
    if (Number.isFinite(metros) && metros > 0) {
      stats.metersByCategory[row.category] =
        (stats.metersByCategory[row.category] ?? 0) + metros;
      stats.metersTotal += metros;
    }
    marcaData(stats, row.installed_at);
  }

...
import { useNotificationsContext } from '@/components/andamento-obra/works/NotificationsBellRealtimeProvider';
import type { NotificationRow } from '@/types/works';

/**
 * Sino de notificações do Quadro de Trabalho. Mesma UI de `WorkNotificationsBell`
 * (`src/components/andamento-obra/works/WorkNotificationsBell.tsx`) — duplicado
 * em vez de parametrizado porque o único ponto que muda é o link de "Ver tudo"
 * (aqui `/tarefas`, lá `/tools/andamento-obra/notificacoes`), e os dois módulos
 * já seguem esse padrão de bell próprio por módulo.
 */
export function TaskNotificationsBell() {
  const router = useRouter();
  const { items, unreadCount, markAsRead, markAllAsRead, pulse } = useNotificationsContext();

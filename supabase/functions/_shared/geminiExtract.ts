...
import type { GeminiExtractResult, SupplierExtractItem } from './types.ts';
...
export async function extractSupplierQuoteWithGemini(
  ...
  const apiKey = apiKeyOverride?.trim() || Deno.env.get('GEMINI_API_KEY');
  if (!apiKey) {
  ...
  return {
    ...
    data: {
      items: parsed.items,
      observacoesGerais,
      quoteDate: normalizeQuoteDate(parsed.quoteDate),
    },
  };
}
